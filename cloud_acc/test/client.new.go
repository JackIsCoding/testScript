...
func requsetUpload(filePath string, blockSize int, offset int64) {
	addrs := "http://10.10.191.3:8886/request_upload?g=10303F67B89C5F132AE727CB67C8B7062A2610F4&ui=123&ak=123&pk=344&s=123&e=1699669994&ms=100"
...
func uploadData(filePath string, blockSize int, offset int64) {
	addrs := "http://10.10.191.3:8886/upload_data?g=10303F67B89C5F132AE727CB67C8B7062A2610F4&ui=123&ak=123&pk=344&s=123&e=1699669994&ms=100"
	f, err := os.Open(filePath)
	defer f.Close()
	if err != nil {
		fmt.Println("Opem file failed", err)
...
func usage() {
	fmt.Println("usage:uploader_client filePath blockSize offset {1(request_upload)|2(upload_data)}")
}
...
func main() {
	...
	fmt.Println(filePath, blockSize)

	if oper == 1 {
		requsetUpload(filePath, blockSize, int64(offset))
	} else {
		uploadData(filePath, blockSize, int64(offset))
	}
}
