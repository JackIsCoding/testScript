...
if __name__ == "__main__":
    ...
    try:
        options, args = getopt.getopt(sys.argv[1:], "h:p:u:c:g:f:b:i:e:z:", ["host=", "port=", "url=", "cid=", "gcid=", "file=", "bt=", "index=", "emule=", "filesize="])
    ...
    for key, value in options:
        if key in ("-f", "--file"):
            ...
            cf = ConfigObj(query_file)
            cf["url"]["query_url"] = "string:" + value
            cf.write()

        elif key in ("-c", "--cid"):
            if query_file == "":
                query_file = "queryresinfo_cid.query"
            cf = ConfigObj(query_file)
            cf["cid_info"]["cid"] = "string_hex:" + value
            cf.write()

        elif key in ("-g", "--gcid"):
            if query_file == "":
                query_file = "querybcid.query"
            cf = ConfigObj(query_file)
            cf["globalsection"]["gcid"] = "string_hex:" + value
            cf.write()

        elif key in ("-b", "--bt"):
            if query_file == "":
                query_file = "querybtinfo.query"
            cf = ConfigObj(query_file)
            cf["globalsection"]["infoid"] = "string_hex:" + value
            flag = 0
            for k, v in options:
                if k in ("-i", "--index"):
                    cf["globalsection"]["index"] = "uint32:" + v
                    flag = 1
                    break;
            if flag == 0:
                print "To query bt, index needed."
                sys.exit()
            cf.write()

        elif key in ("-e", "--emule"):
            if query_file == "":
                query_file = "queryemuleinfo.query"
            cf = ConfigObj(query_file)
            cf["globalsection"]["infoid"] = "string_hex:" + value
            flag = 0
            for k, v in options:
                if k in ("-z", "--filesize"):
                    cf["globalsection"]["filesize"] = "uint64:" + v
                    flag = 1
                    break;
            if flag == 0:
                print "To query emule, filesize needed."
                sys.exit()
            cf.write()
    ...
    print '\n\n------------sending %s to host:%s port:%d\n'%(query_file, host, port)
    #shub_cli = SHubClient('58.61.39.245', 80, query_file, resp_file)
    #shub_cli = SHubClient('58.61.39.214', 80, query_file, resp_file)
    #shub_cli.start()

    shub_cli_test = SHubClient(host, port, query_file, resp_file)
    #shub_cli_test = SHubClient('10.10.159.15', 80, query_file, resp_file)
    shub_cli_test.start()
