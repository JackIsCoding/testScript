...
class Resp:
    ...
    def decode(self, recv_buff):
        ...
        #print 'header_12:',org_buf.read(12)
        cf = ConfigObj(self.conf_file)
        try:
...
class SHubClient:
    ...
    def send_and_recv(self, send_buff):
        try:
            ...
            self._sock.connect(('10.10.159.53', 80))
    ...
    def start_song(self, my_cf={}):
        ...
        if my_cf == {}:
            ...
            send_buff = query.encode(False, my_cf)
        s=requests.Session()
        #res_data = requests.post(url=self._url,data=send_buff,stream=True)
        res_data = s.post(url=self._url,data=send_buff,stream=True)
        #print res_data.headers
        #res_data = urllib2.urlopen(req)
        recv_buff = res_data.raw.read()
        #print 'recv_buff:',recv_buff
        resp = Resp(self._resp_conf)
        return resp.decode(recv_buff)
        #print resp.decode(recv_buff)
