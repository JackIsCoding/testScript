...
class PHubClient:
    ...
    def http_send_recv(self, send_buff):
        ...
        req.add_header("Channel" ,"http-udef")
...
if __name__ == "__main__":
    if len(sys.argv) == 1:
        ...
        -u --url <url of request>                          input your request url'''
        sys.exit()

    try:
	options, args = getopt.getopt(sys.argv[1:], "f:u:", ["file=","url="])
    ...
    print '\n----------sending %s ----------\n'%(query_file)
    print '----------------request:'
    phub_cli_test = PHubClient(url, query_file, resp_file)
    phub_cli_test.start()
