...
class Resp:
    ...
    def write_conf(self, ar, cf, section):
		...
		#print item
                ...
                for i in range(int(val_list[2])):
                    cf["%s_%s_%d"%(section,val_list[1],i)] = copy.copy(cf[val_list[1]])    #set xxx_0  xxx_1
                    #print "=====================%s_%d"%(val_list[1],i)
                    if val_list[1] != 'ipinfo':
                        ar.readI32()
                    self.write_conf(ar, cf, "%s_%s_%d"%(section,val_list[1],i))
            else:
    ...
    def decode(self, recv_buff):
        ...
        # create archive
        org_buf.seek(0)
        data =  org_buf.read(12)+decryptdata 
...
class SHubClient:
	...
	#print req
        ...
        req.add_header("host","%s"%hostname)
...
if __name__ == "__main__":
    ...
    try:
        options, args = getopt.getopt(sys.argv[1:], "f:h:p:u:c:g:s:b:i:e:z:n:", ["file","host=", "port=", "url=", "cid=", "gcid=", "bt=", "index=", "emule=", "filesize="])
    except getopt.GetoptError, err:
        print "opt error: %s"%(err)
        sys.exit()

    query_file = ""
    #host = "http://58.61.39.135"
    host = "http://10.10.159.53:8800"
    #host = "http://t03145s1.sandai.net"
    #host = "http://c1278.sandai.net"
    port = 80
    hostname = "idx.m.hub.sandai.net"
