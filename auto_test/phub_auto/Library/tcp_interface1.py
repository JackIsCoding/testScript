...
class Query:
    ...
    def encode(self,my_cf={}):
        ...
        print "to_be_encrypt len: %d" %len(to_be_encrypt)
...
class Resp:
    ...
    def decode(self, recv_buff):
        ...
        logger.debug("decryptdata len: %d"%len(decryptdata))
...
if __name__ == "__main__":
    if len(sys.argv) == 1:
        ...
        -h --host <domain or ip>                           PHUB's domain or ip (default 10.10.191.79)
        -p --port <port>                                   PHUB's port (defualt is 80)'''
        sys.exit()

    try:
        options, args = getopt.getopt(sys.argv[1:], "f:h:p:", ["host=", "port=", "file="])
