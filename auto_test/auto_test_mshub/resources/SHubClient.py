...
class Query:
    ...
    def encode_with_zlib(self, auto_read=True, my_cf={}):
        ar = OArchive()
        cf = {}
        if auto_read:
            cf = ConfigObj(self.conf_file)
        ...
        # ZLIB compress
        org_buf.seek(0)
        protocol_version, = unpack("I", org_buf.read(4))
        #print "protocol version: %d" %(protocol_version)
        if protocol_version >= 50:
            org_buf.seek(16)
            compress_flag,  = unpack("H", org_buf.read(2))
            #print "compress flag: %d" %(compress_flag)
            need_compress = ((compress_flag >> 8) & 0x1) > 0
            if need_compress:
                print "need compress request"
                offset = 20
                if protocol_version >= 60:
                    org_buf.seek(18)
                    comm_reserve_len, = unpack("I", org_buf.read(4))
                    offset = offset + 4 + comm_reserve_len

                #print "offset: %d" %(offset)
                org_buf.seek(offset)
                to_be_compress = org_buf.read()

                #print 'before compress, command length: %d' %(len(to_be_compress))
                compressdata = zlib.compress(to_be_compress)
                #print 'after compress, command length: %d' %(len(compressdata))

                # set command length
                org_buf.seek(8)
                org_buf.write( pack("I", len(compressdata)) )

                # total buffer
                org_buf.seek(offset)
                org_buf.write(compressdata)
                org_buf.truncate(offset + len(compressdata))
...
class Resp:
    ...
    def write_conf(self, ar, cf, section):
        ...
        for item in cf[section]:
            ...
            if item == "result" and val_list[1] == "0":
                break;
            if item == "cmdid":
                self.cmdid_ = int(val_list[1])
            if item == "has_record" and self.cmdid_ == 5002 and val_list[1] == "0":
                ...
                if ar.readNormal(val_list):
                    ...
                    self.write_conf(ar, cf, val_list[0]) #read user-defined type
                break;
            if item == "has_record" and self.cmdid_ == 4002 and val_list[1] == "0":
                item = 'download_strategy'
                val = cf[section][item]
                val_list = val.split(':')
                if ar.readNormal(val_list):
    ...
    def decode_with_zlib(self, recv_buff, auto_read=True, my_conf={}):
        org_buf = StringIO(recv_buff)
        ...
        decryptdata = decryptor.decrypt(encryptdata)
        org_buf.seek(12)
        org_buf.write(decryptdata)
        org_buf.truncate(12 + len(decryptdata))
        ...
        if protocol_version >= 50:
            ...
            need_compress = ((compress_flag >> 8) & 0x1) > 0
            if need_compress:
                #print "need decompress response"
                offset = 20
                if protocol_version >= 60:
                    org_buf.seek(18)
                    comm_reserve_len, = unpack("I", org_buf.read(4))
                ...
                #print "offset: %d" %(offset)
                org_buf.seek(offset)
                to_be_decompress = org_buf.read()

                #print 'before decompress, command length: %d' %(len(to_be_decompress))
                decompressdata = zlib.decompress(to_be_decompress)
                #print 'after decompress, command length: %d' %(len(decompressdata))

                # set command length
                org_buf.seek(8)
                org_buf.write( pack("I", len(decompressdata)) )

                # total buffer
                org_buf.seek(offset)
                org_buf.write(decompressdata)
                org_buf.truncate(offset + len(decompressdata))
...
class SHubClient(object):
    def __init__(self, url, query_conf=None, resp_conf=None):
        self._url = url
    ...
    def start2(self, req, resp):
        try:
            ...
            return resp_agent.decode(recv_buff, False, resp), 200
    ...
    def start_with_zlib(self, req, resp):
        try:
            req["globalsection"]["compress_flag"] = "uint16:256"
            send_buff = ''
            query = Query(None)
            encode_buff = query.encode_with_zlib(False, req)
            rsa_dealer = RsaDealer()
            send_buff = rsa_dealer.encrypto(encode_buff)
            print "send buffer length: ", len(send_buff)

            req = urllib2.Request(url=self._url,data=send_buff)
            res_data = urllib2.urlopen(req)
            recv_buff = res_data.read()
            print "recv buffer length:", len(recv_buff)

            decrypto_buff = rsa_dealer.decrypto(recv_buff)

            resp_agent = Resp(None)
            return resp_agent.decode_with_zlib(decrypto_buff, False, resp)
        except urllib2.HTTPError as e:
            return None, e.getcode()
        except EOFError as e:
            return None
...
if __name__ == "__main__":
    ...
    query_conf = ConfigObj(query_file)
    resp_conf = ConfigObj(resp_file)
    resp = shub_cli_test.start_with_zlib(query_conf,resp_conf)
    #resp = shub_cli_test.start(query_conf)
    print "=====================================END===================="
    print resp
