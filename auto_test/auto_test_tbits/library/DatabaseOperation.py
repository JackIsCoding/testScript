...
class DatabaseOperation(Database):
	...
	def get_table_suffix(self,table_name,param):
		sql = "select table_num from tbits_db_setting.tbits_hash_table_info_map where table_name = '%s'" %(table_name)
	...
	def get_table_database(self,table_name,suffix):
		sql = "select * from tbits_db_setting.tbits_hash_table_db_map where table_name = '%s'" %(table_name)
		affect , result = self.execute_sql_string(sql)
		suffix = int(suffix)
		db_name = None
		for data in result:
	...
	def delete_check(self,sql):
		...
		if affect == 0:
			...
			return False
#####tbits##########
	def update_amount_info_up(self,userid):
		#table_name = self.get_full_table('user_account',userid)
		sql = "update tbits_db_1.user_account_4 set amount=1000 where userid = %d" % (int(userid))
		affect , result = self.execute_sql_string(sql)
		#sql = "select amount from user_account_4 where userid  = %d" % (int(userid))
		#return self.update_check(sql)
	
	def update_amount_info_down(self,userid):
		#table_name = self.get_full_table('user_account',userid)
		sql = "update tbits_db_1.user_account_0 set amount=1000 where userid = %d" % (int(userid))
		affect , result = self.execute_sql_string(sql)
		#sql = "select amount from user_account_4 where userid  = %d" % (int(userid))
		#return self.update_check(sql)
	def delete_user_info(self,userid):
		sql = "delete from tbits_db_1.user_account_3 where userid = %d" % (int(userid))
		affect , result = self.execute_sql_string(sql)
##########################	
