...
class PHubClient:
    def __init__(self, query_conf, resp_conf):
        self._query_conf = query_conf
        self._resp_conf = resp_conf
	print("query_conf:", self._query_conf)
	print("resp_conf:", self._resp_conf)
    
    def get_rcv_buff(self, rcv_buff):
        org_buf = StringIO(rcv_buff)
        md = org_buf.read(8)
        org_buf.seek(8)
        encryptdata_len = unpack("I",org_buf.read(4))
        org_buf.seek(12)
    ...
    def http_send_recv(self, send_buff):
	#test_unified_gateway
	#req = urllib2.Request(url="http://10.10.191.2:80/phub/v1.0/report", data=send_buff) 
        #req.add_header("Channel" ,"http-json")
        #req.add_header("Host","hub5pr.sandai.net")
	
	#test_connection
	req = urllib2.Request(url="http://m07a025.sandai.net:3076", data=send_buff)
	req.add_header("x-forwarded-for", "10.10.32.145")
        
	resp = urllib2.urlopen(req)
        rcv_buff = resp.read()
        return rcv_buff #self.get_rcv_buff(rcv_buff)

    def start(self):
        query = Query(self._query_conf)
        send_buff = query.encode()
        recv_buff = self.http_send_recv(send_buff)
        if recv_buff == None:
            return None
...
if __name__ == "__main__":
    ...
    print '\n----------sending %s ----------\n'%(query_file)
    print '----------------request:'
    phub_cli_test = PHubClient(query_file, resp_file)
    phub_cli_test.start()
