...
class PHubClient:
    ...
    def send_and_recv(self, send_buff):
        try:
            ...
            self._sock.sendto( send_buff, addr )

            '''
            twelve_bytes_header = self._sock.recvfrom(12)
            ...
            body_buff = self._sock.recvfrom(command_length)
			'''
...
if __name__ == "__main__":
    if len(sys.argv) == 1:
        ...
        -h --host <domain or ip>                           PHUB's domain or ip (default 10.10.191.60)
        -p --port <port>                                   PHUB's port (defualt is 80)'''
        sys.exit()

    try:
		options, args = getopt.getopt(sys.argv[1:], "f:h:p:", ["file=", "host=", "port="])
    ...
    #shub_cli = PHubClient('10.10.191.60', 8000, query_file, resp_file)
    #shub_cli = PHubClient('58.61.39.214', 80, query_file, resp_file)
    #shub_cli.start()

    #print '----------------test'
    print "host: %s, port: %s" %(host, port)
    shub_cli_test = PHubClient(host, port, query_file, resp_file)
    #shub_cli_test = PHubClient('10.10.13.31', 80, query_file, resp_file)
    shub_cli_test.start()
